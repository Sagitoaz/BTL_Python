...
def complete(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        r.raise_for_status()
        data = r.json()
        completion = data.get("completion") or (data.get("choices") or [{}])[0].get(
            "text", ""
        )
...
def main():
    log(
        f"[bridge.py] start SERVER_URL={SERVER_URL} API_KEY={'set' if API_KEY else 'empty'} TIMEOUT={TIMEOUT}s"
    )
    try:
